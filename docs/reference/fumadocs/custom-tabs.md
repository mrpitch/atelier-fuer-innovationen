...
description: "src/components/tabs.tsx is a ~1.5-year-old local reimplementation of Tabs features @fumadocs/base-ui now covers natively — prefer the built-in Tabs going forward."
kind: reference
type: Reference
status: current
...
**This was built ~1.5 years ago because fumadocs-ui's own `Tabs` didn't support this at the time.** As of the current `@fumadocs/base-ui` (the package this repo now uses in place of `fumadocs-ui`), the built-in `Tabs` component (`@fumadocs/base-ui/components/tabs` or similar) now supports the same `groupId`, `persist`, and `updateAnchor` props natively.

- **Prefer `@fumadocs/base-ui`'s built-in `Tabs` for any new usage** rather than this local component.
- Migrating existing usages of `src/components/tabs.tsx` to the built-in component is a worthwhile follow-up cleanup, not done as part of this standard — verify prop-for-prop parity against the current `@fumadocs/base-ui` docs before swapping, since exact persistence/hash-sync semantics should be checked rather than assumed identical.
