...
- The color variable is **opt-in per top-level content folder** (e.g. `--atelier-color`, `--xeniapolis-color` for `src/content/docs/atelier/`, `src/content/docs/xeniapolis/`). Both are now defined, in `src/lib/styles/globals.css`, wired to each palette's `--primary` value at `:root`/`.dark` — see [ADR-0002](../../adr/0002-per-section-theming.md) and [About section theming](../../explanation/about-section-theming.md). They're deliberately set outside the `[data-section="xeniapolis"]` scope, so the switcher always shows both identities regardless of the current page.
...
- `layout.tsx` lives at `src/app/docs/[[...slug]]/layout.tsx`, not `src/app/docs/layout.tsx`. It was moved into the dynamic segment folder so it shares route params with `page.tsx` — a parent-level `docs/layout.tsx` only ever receives params for its own (static) segment, never the child `[[...slug]]` segment, so it couldn't otherwise know the current page to derive `data-section` for it (see ADR-0002).
