...
- `scripts/run-sandcastle.ts` and whichever of `.sandcastle/Dockerfile`, `.sandcastle/prompt.md`, `.sandcastle/.env.example`, `.sandcastle/.gitignore`, repo-root `.dockerignore` are being newly written (only when Section B's sandbox-command item accepted a scaffold offer, full or fix-forward) — with the package-manager placeholders already substituted, so the user reviews what will actually be written, not the raw template
- When Section B item 1's private-registry or packageManager-pin checks fired: the `--mount=type=secret` `Dockerfile` variant, the `.gitignore`/`git rm --cached` proposal, the image-build command, and/or the `package.json` `packageManager` addition
- The diff for `to-tickets/SKILL.md` and/or `to-spec/SKILL.md` (only when Section B's skill-hygiene check found the hardcoded-label pattern)
...
- [sandcastle-dockerfile](./templates/sandcastle/sandcastle-dockerfile), [sandcastle-prompt.md](./templates/sandcastle/sandcastle-prompt.md), [sandcastle-env-example](./templates/sandcastle/sandcastle-env-example), [sandcastle-gitignore](./templates/sandcastle/sandcastle-gitignore) — written to `.sandcastle/Dockerfile`, `.sandcastle/prompt.md`, `.sandcastle/.env.example`, `.sandcastle/.gitignore` respectively (same condition as `run-sandcastle.ts`; skip any of the four that already exist rather than overwriting them)
- [sandcastle-dockerignore](./templates/sandcastle/sandcastle-dockerignore) — written to the repo root as `.dockerignore` (same condition as `run-sandcastle.ts`; if a `.dockerignore` already exists, merge in the template's patterns rather than overwriting the file). If Section B item 1's private-registry check fired, apply the `--mount=type=secret` Dockerfile variant, the `.gitignore` addition plus `git rm --cached <file>`, and the image-build command update, all as confirmed in step 3. If the packageManager-pin check fired, add the confirmed `packageManager` field to `package.json`.
...
- **If Section B item 1's Sandcastle scaffold offer was accepted, `afk` isn't runnable yet** — three manual steps remain, none of which this skill can do for the user:
  ...
  2. Build the sandbox image once — `npx sandcastle docker build-image`, run from the repo root, requires Docker running.
  3. Add the sandcastle-related commands to this project's Bash permissions (`.claude/settings.json` or `.claude/settings.local.json`), or every `afk` run will stop for manual approval instead of running unattended — e.g.:
     ...
     "Bash(docker build *)"
     ```
     (swap `pnpm tsx *` for whatever runner `sandbox_command` actually uses). The `update-config` skill can wire these in if asked.
