...
1. Sandbox command for `afk` execution — default depends on what the explore step found under `.sandcastle/`:
   - **No `.sandcastle/` at all** — ask whether to scaffold one, default **no**: "This repo has no Sandcastle sandbox set up. Scaffold one for `afk` execution? (default: no — requires Docker and an `@ai-hero/sandcastle` install)". On **no** (or blank), behavior is unchanged from before this item existed: blank default, `afk` runs in-session. On **yes**, write `.sandcastle/Dockerfile`, `.sandcastle/prompt.md`, `.sandcastle/review-prompt.md`, `.sandcastle/efficiency-rules.md`, `.sandcastle/mcp-playwright-connect.sh` (`chmod +x` it), `.sandcastle/.env.example`, `.sandcastle/.gitignore`, the repo-root `.dockerignore`, and the repo-root `.mcp.json` from this skill's [sandcastle-dockerfile](./templates/sandcastle/sandcastle-dockerfile), [sandcastle-prompt.md](./templates/sandcastle/sandcastle-prompt.md), [sandcastle-review-prompt.md](./templates/sandcastle/sandcastle-review-prompt.md), [sandcastle-efficiency-rules.md](./templates/sandcastle/sandcastle-efficiency-rules.md), [sandcastle-mcp-playwright-connect.sh](./templates/sandcastle/sandcastle-mcp-playwright-connect.sh), [sandcastle-env-example](./templates/sandcastle/sandcastle-env-example), [sandcastle-gitignore](./templates/sandcastle/sandcastle-gitignore), [sandcastle-dockerignore](./templates/sandcastle/sandcastle-dockerignore), and [sandcastle-mcp-json](./templates/sandcastle/sandcastle-mcp-json) templates — `prompt.md` and `review-prompt.md` both instruct the agent to read `.sandcastle/efficiency-rules.md` before starting, so it must be written alongside them or that instruction points at a file that doesn't exist — the `.dockerignore` matters even without a private registry: without it, the Dockerfile's final `COPY . .` copies whatever else is sitting untracked-but-present in the repo (a stray `.env`, a credentials file) straight into the image — the root `.mcp.json` wires `playwright-mcp` into every sandcastle run (see `mcp-playwright-connect.sh`'s own comments for why it can't just point at `host.docker.internal` directly): merge in the template's `playwright` entry under `mcpServers` if `.mcp.json` already exists, rather than overwriting the file, the same reasoning as the `.dockerignore` merge just above — plus `scripts/run-sandcastle.ts` from [run-sandcastle.ts](./templates/sandcastle/run-sandcastle.ts) — substituting the `REPLACE_WITH_INSTALL_COMMAND`/`REPLACE_WITH_LOCKFILE_COPY`/`REPLACE_WITH_CHECK_COMMAND` placeholders in those templates for the repo's actual package-manager commands, resolved from the lockfile the explore step found (`pnpm-lock.yaml` → `pnpm`, `package-lock.json` → `npm`, `yarn.lock` → `yarn`; default to `pnpm` if none is present, matching this skillset's own convention). `sandbox_command` becomes `pnpm tsx scripts/run-sandcastle.ts {issue}` (swap `pnpm` for the resolved package manager's runner). `run-sandcastle.ts` imports `@ai-hero/sandcastle`, `@ai-hero/sandcastle/sandboxes/docker`, and `zod` directly, and the `afk` script below invokes bare `tsx` — so also add `@ai-hero/sandcastle`, `tsx`, `zod`, and `@playwright/mcp` to `package.json`'s `devDependencies` (skip any already present, at whatever version they're already pinned to) and run the resolved package manager's install once so the lockfile picks them up. Skipping this leaves both the host `afk` script and the Dockerfile's own `RUN REPLACE_WITH_INSTALL_COMMAND` step failing with the same missing-module error, since the Dockerfile only installs what `package.json` already declares. `@ai-hero/sandcastle` only exposes an `import` export condition (no `require`) — if the repo root's `package.json` doesn't already have `"type": "module"`, tsx treats `run-sandcastle.ts` as CommonJS and transpiles its import to `require()`, which then fails against that package. Check the root `package.json`'s `type` field: if it's already `"module"`, nothing extra is needed; otherwise write `scripts/package.json` with `{"type": "module"}` (skip if that file already exists with the field set) — this scopes ESM resolution to just `scripts/` without touching the root config, which other tooling in the repo (e.g. Next.js) may depend on staying CommonJS. Also add two convenience scripts to `package.json`'s `scripts`: `"afk": "tsx --env-file=.sandcastle/.env scripts/run-sandcastle.ts"` — a human-facing wrapper for manual `<pm> run afk <issue>` invocation, loading the sandbox's env vars, unlike `sandbox_command` itself, which relies on the calling Claude Code session's own environment already carrying them — and `"sandcastle:build": "DOCKER_BUILDKIT=1 docker build --build-arg AGENT_UID=$(id -u) --build-arg AGENT_GID=$(id -g) -t sandcastle:<repo-slug> -f .sandcastle/Dockerfile ."` (`<repo-slug>` from `package.json`'s `name` field, lowercased and non-alphanumeric characters replaced with `-`, or the repo directory name if `name` is missing). Building through this script rather than `npx sandcastle docker build-image` matters even without a private registry — it's the one place the `--secret` flag below can be added without inventing a second build path later. If either script already exists, show the diff in the step-3 draft instead of silently overwriting — the repo may have deliberately customized it.
   - **`.sandcastle/` present, with exactly one `package.json` script matching it** — judge whether that script's own argument parsing needs only the issue number (optionally a `--repair-context <path>` flag), matching `implement-epic`'s `sandbox_command` contract (substitutes `{issue}` and nothing else; a configured command should support `--repair-context <path>` for repair passes). This is a read-the-script judgment call, like the label look-alike matching in the label-ensure step below — not a formal parser.
     ...
     - **Doesn't fit** (needs more than the issue number, e.g. a script taking `<issueNumber> <issueTitle> <issueBody> <targetBranch>`) — do not guess a broken command. Propose leaving `sandbox_command` blank, explain the mismatch (what the script actually needs vs. what `implement-epic` will pass it), and in the same turn offer the scaffold from the no-`.sandcastle/` case above as a fix-forward alternative — reusing the existing `.sandcastle/Dockerfile`/`prompt.md`/`review-prompt.md`/`efficiency-rules.md`/`mcp-playwright-connect.sh`/`.env.example`/repo-root `.mcp.json` if present rather than overwriting them, writing only `scripts/run-sandcastle.ts` (and whichever `.sandcastle/` files, the repo-root `.dockerignore`, and the repo-root `.mcp.json` are actually missing), plus the `afk`/`sandcastle:build` `package.json` scripts, the `@ai-hero/sandcastle`/`tsx`/`zod`/`@playwright/mcp` `devDependencies`, and the `scripts/package.json` ESM fix, all described above — whichever aren't already present.
   ...
   - **`.sandcastle/` present, no matching script found** — offer to scaffold `scripts/run-sandcastle.ts` alone from the [run-sandcastle.ts](./templates/sandcastle/run-sandcastle.ts) template (same placeholder substitution as above), reusing whatever `.sandcastle/` assets already exist rather than guessing a generic invocation of an entrypoint that may not exist in the shape assumed. Also offer the repo-root `.dockerignore` from [sandcastle-dockerignore](./templates/sandcastle/sandcastle-dockerignore), the `afk`/`sandcastle:build` `package.json` scripts, the `@ai-hero/sandcastle`/`tsx`/`zod` `devDependencies`, and the `scripts/package.json` ESM fix, all described above — whichever don't already exist.
...
- `scripts/run-sandcastle.ts` and whichever of `.sandcastle/Dockerfile`, `.sandcastle/prompt.md`, `.sandcastle/review-prompt.md`, `.sandcastle/efficiency-rules.md`, `.sandcastle/mcp-playwright-connect.sh`, `.sandcastle/.env.example`, `.sandcastle/.gitignore`, repo-root `.dockerignore`, repo-root `.mcp.json` are being newly written or merged (only when Section B's sandbox-command item accepted a scaffold offer, full or fix-forward) — with the package-manager placeholders already substituted, so the user reviews what will actually be written, not the raw template
- The `package.json` `scripts.afk`/`scripts["sandcastle:build"]` additions (or their diff, if either name already existed), the `@ai-hero/sandcastle`/`tsx`/`zod` `devDependencies` additions, and `scripts/package.json` if the root `package.json` needed the ESM scoping fix
...
- [run-sandcastle.ts](./templates/sandcastle/run-sandcastle.ts) — canonical Sandcastle runner, written to `scripts/run-sandcastle.ts` (only if the user accepted a scaffold offer in Section B's sandbox-command item); substitute the package-manager placeholders before writing
- [sandcastle-dockerfile](./templates/sandcastle/sandcastle-dockerfile), [sandcastle-prompt.md](./templates/sandcastle/sandcastle-prompt.md), [sandcastle-review-prompt.md](./templates/sandcastle/sandcastle-review-prompt.md), [sandcastle-efficiency-rules.md](./templates/sandcastle/sandcastle-efficiency-rules.md), [sandcastle-mcp-playwright-connect.sh](./templates/sandcastle/sandcastle-mcp-playwright-connect.sh), [sandcastle-env-example](./templates/sandcastle/sandcastle-env-example), [sandcastle-gitignore](./templates/sandcastle/sandcastle-gitignore) — written to `.sandcastle/Dockerfile`, `.sandcastle/prompt.md`, `.sandcastle/review-prompt.md`, `.sandcastle/efficiency-rules.md`, `.sandcastle/mcp-playwright-connect.sh` (`chmod +x` it), `.sandcastle/.env.example`, `.sandcastle/.gitignore` respectively (same condition as `run-sandcastle.ts`; skip any of the seven that already exist rather than overwriting them)
- [sandcastle-mcp-json](./templates/sandcastle/sandcastle-mcp-json) — written to the repo root as `.mcp.json` (same condition as `run-sandcastle.ts`); if `.mcp.json` already exists, merge the template's `playwright` entry into its `mcpServers` object rather than overwriting the file — the same reasoning as the `.dockerignore` merge just below
- [sandcastle-dockerignore](./templates/sandcastle/sandcastle-dockerignore) — written to the repo root as `.dockerignore` (same condition as `run-sandcastle.ts`; if a `.dockerignore` already exists, merge in the template's patterns rather than overwriting the file). If Section B item 1's private-registry check fired, apply the `--mount=type=secret` Dockerfile variant, the `.gitignore` addition plus `git rm --cached <file>`, and the `sandcastle:build` script's `--secret` addition, all as confirmed in step 3. If the packageManager-pin check fired, add the confirmed `packageManager` field to `package.json`.
- `package.json`'s `scripts.afk` and `scripts["sandcastle:build"]` (same condition as `run-sandcastle.ts`) — parse the file, add or update just these two keys, leave every other key untouched, and re-serialize preserving the file's existing indentation. Same condition, add `@ai-hero/sandcastle`, `tsx`, `zod`, and `@playwright/mcp` to `devDependencies` (skip any already present), then run the resolved package manager's install command once so the lockfile is updated to match — without this, both the host `afk` script and the sandbox image's own frozen-lockfile install fail with a missing-module error; for `@playwright/mcp` specifically, `mcp-playwright-connect.sh` invokes it with `npx --no-install`, which fails outright (no network fallback) if it isn't already in the container's `node_modules`. Same condition, if the root `package.json` doesn't have `"type": "module"`, write `scripts/package.json` with `{"type": "module"}` — skip this if `scripts/package.json` already exists with that field set.
...
- **If Section B item 1's Sandcastle scaffold offer was accepted, `afk` isn't runnable yet** — three manual steps remain, none of which this skill can do for the user:
  ...
  3. Add the sandcastle-related commands to this project's Bash permissions (`.claude/settings.json` or `.claude/settings.local.json`), or every `afk` run will stop for manual approval instead of running unattended — e.g.:
     ```json
     "Bash(pnpm run sandcastle:build)",
     "Bash(pnpm tsx *)"
     ```
     (swap `pnpm tsx *` for whatever runner `sandbox_command` actually uses; the Bash permission check matches the literal command Claude Code types, so `sandcastle:build`'s own internal `docker build` call needs no separate rule). The `update-config` skill can wire these in if asked.
- **`playwright-mcp` needs a real Chrome (or Chromium) installed on the machine running `afk`/`hitl` sessions** — `run-sandcastle.ts` launches it directly on the host for every run (see the "Host Chrome for Playwright MCP" section of that file); it isn't bundled into the sandbox image or installed by this skill. Without it, `findChromeBinary()` throws before the container even starts. This only applies to the machine actually running sandcastle — not every clone of the repo.
